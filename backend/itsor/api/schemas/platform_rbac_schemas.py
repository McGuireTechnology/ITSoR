...
from itsor.domain.models import PlatformPermissionEffect, PlatformResourceAction


def _coerce_platform_resource_action(value):
    if isinstance(value, PlatformResourceAction):
        return value
    if isinstance(value, int):
        return PlatformResourceAction.from_nibble(value)
    if isinstance(value, str):
        trimmed = value.strip().lower()
        if trimmed.isdigit():
            return PlatformResourceAction.from_nibble(int(trimmed))
        return PlatformResourceAction.from_verb(trimmed)
    return value
...
class PlatformPermissionCreate(BaseModel):
    ...
    effect: PlatformPermissionEffect = PlatformPermissionEffect.ALLOW

    @field_validator("action", mode="before")
    @classmethod
    def validate_action(cls, value):
        return _coerce_platform_resource_action(value)
...
class PlatformPermissionUpdate(BaseModel):
    ...
    effect: PlatformPermissionEffect | None = None
