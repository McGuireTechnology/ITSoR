...
async function handleSave() {
  if (!namespace.value || !canWrite.value) {
    return
  }
...
async function handleDelete() {
  ...
  try {
    ...
    await router.push('/customization/namespaces')
  } catch (deleteError) {
...
<template>
  <section class="panel card shadow-sm border-0 rounded-4 p-4 bg-brand-surface/70">
    <h2 class="h3 fw-bold mb-3 text-brand-deep">Namespace Detail</h2>
    ...
    <template v-else>
      ...
      <dl class="user-detail section-gap">
        ...
        <dt>Owner</dt>
        <dd>{{ namespace.owner_id || '-' }}</dd>
        <dt>Group</dt>
        <dd>{{ namespace.group_id || '-' }}</dd>
        <dt>Permissions</dt>
        <dd>{{ namespace.permissions ?? '-' }}</dd>
      </dl>
