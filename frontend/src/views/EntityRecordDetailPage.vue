...
async function handleSave() {
  if (!entityRecord.value || !canWrite.value) {
    return
  }
...
async function handleDelete() {
  ...
  try {
    ...
    await router.push('/customization/entity-records')
  } catch (deleteError) {
...
<template>
  <section class="panel card shadow-sm border-0 rounded-4 p-4 bg-brand-surface/70">
    <h2 class="h3 fw-bold mb-3 text-brand-deep">Entity Record Detail</h2>
    ...
    <template v-else>
      <form class="form" @submit.prevent="handleSave">
        <label>
          Name
          <input v-model="name" type="text" :disabled="!canWrite || saving" />
        </label>
        <label>
          Values JSON
          <textarea v-model="valuesJsonText" rows="8" required :disabled="!canWrite || saving" />
        </label>
      ...
      <dl class="user-detail section-gap">
        ...
        <dt>Owner</dt>
        <dd>{{ entityRecord.owner_id || '-' }}</dd>
        <dt>Group</dt>
        <dd>{{ entityRecord.group_id || '-' }}</dd>
        <dt>Permissions</dt>
        <dd>{{ entityRecord.permissions ?? '-' }}</dd>
      </dl>
